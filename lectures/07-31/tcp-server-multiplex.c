...
{
  fd_set readfds;
  int client_fds[MAX_CLIENTS]; /* client socket fd list */
  int client_fd_index = 0;     /* next free spot */
  ...
  while ( 1 )
  {
    ...
    for ( int i = 0 ; i < client_fd_index ; i++ )
    {
    ...
    /*  (return value will be 1 or greater)                   */
    int ready = select( FD_SETSIZE, &readfds, NULL, NULL, NULL );
#else
    int ready = select( FD_SETSIZE, &readfds, NULL, NULL, &timeout );

    if ( ready == 0 ) { printf( "SERVER: No activity (yet)...\n" ); continue; }
#endif
    ...
    if ( FD_ISSET( listener, &readfds ) )
    {
      ...
      client_fds[client_fd_index++] = newsd;
    }
    ...
    for ( int i = 0 ; i < client_fd_index ; i++ )
    {
      ...
      if ( FD_ISSET( fd, &readfds ) )
      {
        ...
        else if ( n == 0 )
        {
          ...
          /* remove fd from client_fds[] array: */
          for ( int k = 0 ; k < client_fd_index ; k++ )
          {
            if ( fd == client_fds[k] )
            {
              /* found it -- copy remaining elements over fd */
              for ( int m = k ; m < client_fd_index - 1 ; m++ )
              {
                client_fds[m] = client_fds[m + 1];
              }
              client_fd_index--;
              break;  /* all done */
            }
